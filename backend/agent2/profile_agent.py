...
# FastAPI 라우터 인스턴스 생성 (main.py에서 임포트 가능하도록 구성)
router = APIRouter(
...
class ProfileDiagnosis(BaseModel):
    # 1) 온보딩 원본 데이터 보존 필드 (에이전트 3에서의 갭 분석을 위해 온전히 복사하여 전달)
    ...
    # 2) LLM 분석 진단 필드 (정량적 수치 편향 배제, 순수 정성 분석만 전달)
...
# 3. 비동기식 Upstage API 호출 핵심 비즈니스 로직 (메인 조립 파이프라인에서 import하여 사용 가능)
...
# 4. FastAPI 엔드포인트 노출 (main.py에 include_router 시 자동 마운트됨)
